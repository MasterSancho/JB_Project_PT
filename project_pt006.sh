...
echo "1) Use default lists"
echo "2) Use custom lists"
read list_choice

if [ "$list_choice" == "2" ]; then
    echo "Enter the full path to your custom username list (hit enter for default):"
    read custom_user_list
    echo "Enter the full path to your custom password list (hit enter for default):"
    read custom_pass_list
    user_list_path=${custom_user_list:-"./lists/userlist.lst"}
    pass_list_path=${custom_pass_list:-"./lists/passlist.lst"}
else
...
perform_brute_force() {
    ...
    echo "Starting brute force attack on $service for $ip..."
